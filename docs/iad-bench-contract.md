...
IAD-Risk 需要同时使用公开 gold、proxy benchmark、weak / silver label、LLM 辅助标签和后续人工审查。若这些标签混在同一个指标里，审稿人会质疑结论不可信。
...
禁止把 OpenAlex、SciRepEval、GPT 或引用邻近关系写成 gold label。
...
| `llm_silver` | GPT 或其他 LLM 辅助判断 | 否 | 数据增强和解释分析 |
| `human_audit` | 后续人工审查 | 可作为独立 audit gold | 期刊增强和误差分析 |
