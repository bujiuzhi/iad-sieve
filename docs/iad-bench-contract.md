...
IAD-Risk 需要同时使用公开 gold、proxy benchmark、weak / silver label 和人工复核标签。若这些标签混在同一个指标里，读者无法判断结论的证据强度。
...
禁止把 OpenAlex、SciRepEval、引用邻近关系或其他弱监督信号写成 gold label。
...
| `label_strength` | `gold`、`distant`、`proxy`、`silver`、`human_audit` |
...
用于阈值选择和模型选择。不得把单一弱监督来源作为唯一 dev 信号。
