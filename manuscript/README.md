...
外部 artifact release 应发布可再分发的派生表格、预测文件、日志、manifest 和 checksum，而不是原始第三方来源文件，除非原始来源条款明确允许再分发。`configs/source_input_manifest.json` 必须记录每个公开输入的 original provider、acquisition date or version、local file boundary、license boundary 和 SHA256 checksum。
