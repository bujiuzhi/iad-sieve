...
`target_journal_shortlist.md` 记录候选期刊、适配风险和模板前置要求。该文件用于投稿前决策，不作为最终期刊系统上传附件；正式上传前仍需由作者确认目标期刊、真实作者指南来源 URL 和最新分区/分类信息，并在 `submission_metadata.yml` 中填写 `ranking_confirmation_completed`、`ranking_confirmation_source`、真实 `ranking_confirmation_source_url`、`ranking_confirmation_checked_date` 和 `selected_target_author_confirmed`。若选择 DKE/Elsevier 路线，还需在最终上传前准备作者 biography 和 passport-type photograph 等作者身份材料，并在 `author_identity_materials` 中记录 `biography_files`、`photograph_files` 和 `author_identity_materials_verified`。
