...
The main Open-v2 result artifact is \path{open_v2_main_results}. Its CSV file must include per-row denominator counts, the per-row threshold source, and the scope label used in the main table. This row-level schema lets reviewers trace each reported same-work F1, FMR, and HNFMR value to the evaluated pair scope, denominator, operating-point source, and fixed artifact version.
