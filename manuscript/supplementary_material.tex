...
Thresholds turn a classifier into an unsafe automatic merger. & Expose work, agenda-non-identity, and risk thresholds and evaluate held-out merge decisions. & Threshold transfer should be rechecked under new source distributions. \\
...
\midrule
$p_{\mathrm{ani}}$ term & Directly represents evidence that a related pair is not the same work. & Requires valid agenda-non-identity supervision or artifact-backed hard-negative evidence. \\
...
Agenda-non-identity risk head & Transformer distances, title and author signals, topic overlap, reference overlap, year conflict, identifier agreement, and different-identifier conflicts. & Agenda-non-identity hard negatives from valid hard-negative sources. & Estimates $p_{\mathrm{ani}}$ as direct blocking evidence. \\
Risk gate & $p_{\mathrm{work}}$, $p_{\mathrm{agenda}}$, $p_{\mathrm{ani}}$, $\tau_w$, $\tau_n$, $\tau_r$, and cannot-link flags. & Computes $p_{\mathrm{risk}}=\max\{p_{\mathrm{ani}},p_{\mathrm{agenda}}(1-p_{\mathrm{work}})\}$. & Emits merge, block, or defer decisions. \\
...
Threshold fixation & Select or record $\tau_w$, $\tau_n$, and $\tau_r$ before held-out reporting. & Threshold source, value, selection split, and selection metric are written to the threshold log. \\
...
2 & Build feature groups without using audit metadata as predictors. & Identity, agenda, agenda-non-identity, and cannot-link feature groups. & Prevents provenance or split leakage. \\
...
5 & Apply the merge gate and cannot-link evidence. & $\tau_w$, $\tau_n$, $\tau_r$, cannot-link flag, and threshold source. & Emits merge, block, or defer under a fixed operating point. \\
...
\midrule
Model complexity & Train and audit same-work, same-agenda, and agenda-non-identity heads under one pair schema. & The net benefit is strongest in high-stakes scholarly indexes where false merges are costly. \\
...
IAD-Bench assembly & Processed source directories from previous stages. & \texttt{build-iad-bench}. & Unified documents, pairs, splits, provenance, and dataset card. & Full package required for L2/L3 artifact release. \\
...
Let $N$ denote the evaluated pair count for one row of the result table, with $M$ automatic merges, $B$ blocked pairs, and $D$ deferred pairs under the same fixed operating point. The release artifact should satisfy $N=M+B+D$ for every row. Automatic merge coverage is $\mathrm{AMC}=M/N$, block rate is $\mathrm{BR}=B/N$, and defer rate is $\mathrm{DR}=D/N$. Let $B_{\mathrm{review}}\leq B$ and $D_{\mathrm{review}}\leq D$ denote the blocked and deferred pairs that the declared workflow sends to human review; terminal cannot-link blocks that need no further adjudication remain counted in $B$ but are not counted in $R$. If $R=B_{\mathrm{review}}+D_{\mathrm{review}}$ and $C$ is the predeclared review capacity for the batch, capacity-normalized review load is $\mathrm{CNRL}=R/C$. These quantities must be derived from the same prediction artifact, threshold configuration, row scope, and denominator record as FMR and HNFMR.
...
Current manuscript claim & Pair-level F1, FMR, and HNFMR under fixed operating points. & Cluster-level contamination claims remain outside the current evidence package. \\
\bottomrule
\end{tabular}
\end{table}
...
IAD-Risk transformer variants & The IAD-Risk rows test split-held-out risk gating with explicit identity, agenda, and agenda-non-identity signals. & The rows are not a claim of broad method superiority without same-scope predictions, intervals, and artifact checksums. \\
Zero-HNFMR IAD-Risk rows & Zero observed HNFMR means no observed false merge in the agenda-hard-negative stratum under the reported scope and operating point, while ordinary FMR is still reported separately. & A zero HNFMR value is not evidence that all non-identity false merges are absent or that risk is zero across sources. \\
...
IAD-Risk transformer variants & Held-out Open-v2 test scope. & Tests whether explicit identity, agenda, and agenda-non-identity heads support risk-gated held-out decisions. & A general ranking over every baseline before all rows share the same released prediction scope. \\
...
\midrule
Merge thresholds & Record $\tau_w$, $\tau_n$, $\tau_r$, validation source, and fixed operating point. & Prevents post-hoc threshold selection on the test set. \\
...
\begin{table}[H]
\caption{Ablation acceptance protocol. The protocol defines when a component-ablation output is strong enough to support causal interpretation rather than exploratory diagnosis.}
\label{tab:ablation-acceptance-protocol}
\centering
\footnotesize
\setlength{\tabcolsep}{2pt}
...
\midrule
Required variants & no-risk-gate, no-ANI-head, single-space, no-cannot-link, and post-hoc-threshold outputs are all present in the \texttt{protocol\_variant} field. & Component causality from only one removed head or one threshold sweep. \\
...
Decision trace & Each variant releases prediction rows, threshold logs, same-work F1/FMR/HNFMR denominators, and merge/block/defer decisions. & Reporting only aggregate metrics without row-level decisions and denominator records. \\
...
IAD-Risk transformer variants & $p_{\mathrm{work}}$, $p_{\mathrm{ani}}$, and $p_{\mathrm{risk}}$ under the risk gate. & Model configuration; default $\tau_w=\tau_n=\tau_r=0.5$ unless the artifact config records an override. & Prediction file, model JSON, thresholds, and checksums. \\
\bottomrule
\end{tabular}
\end{table}

Default-threshold audit rule. A default IAD-Risk operating point is auditable only when the artifact row records \texttt{threshold\_source=predeclared\_default}, a pre-run configuration checksum, a configuration timestamp or run identifier, and evidence that threshold values were fixed before held-out scoring. The default threshold is a fixed rule, not evidence of validation-selected, optimized, or threshold-stable performance without \path{threshold_selection_logs} and \path{threshold_sensitivity_grid} from the same prediction file.
...
\midrule
Fixed operating point & Reported through the operating-point disclosure table and tied to validation or default thresholds. & Configuration file recording $\tau_w$, $\tau_n$, $\tau_r$, baseline thresholds, and selection source. \\
...
Experimental strength & Open-v2 evidence snapshot, operating-point disclosure, and result audit trail. & Current evidence supports targeted pair-level false-merge control under the stated Open-v2 scope. & Stronger ranking claims require same-scope predictions, threshold logs, bootstrap intervals, and released metric tables. \\
...
Method design soundness & Risk score rationale, merge-decision rule, and validity threats. & IAD-Risk exposes identity, agenda, agenda-non-identity risk, thresholds, and cannot-link behavior. & Broad deployment claims require source-heldout validation, topic-heldout checks, and failure-case analysis. \\
\bottomrule
\end{tabular}
\end{table}
...
Method design soundness & Risk gating exposes identity, agenda, agenda-non-identity, threshold, and cannot-link behavior. & Deployment claims require source-heldout validation, topic-heldout checks, review-load analysis, and cluster-level artifact audits. \\
...
Identifier conflict & DOI, arXiv, or OpenAlex identifiers disagree despite text similarity. & Different-identifier conflicts enter the agenda-non-identity risk head and cannot-link logic. & Identifier-resolution logs and conflict audit table. \\
