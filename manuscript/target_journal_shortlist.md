...
| Author biographies and photographs | DKE author guidance requests a short biography of each author with a maximum 100 words, an editable biography file that must not be PDF, and a passport-type photograph as a separate figure. | Collect author-approved biographies and photographs before final upload if DKE remains the selected route. |
...
This lock records DKE official-guide requirements that cannot be inferred from the anonymous preflight package. It is a final-upload control, not a statement that the missing author inputs have already been supplied.

| Official-guide area | Source requirement used for planning | Metadata consequence |
| --- | --- | --- |
| Author list and order | The author list and order should be definitive at original submission, and all authors must be listed in both the manuscript and submission system. | Do not set `final_upload_checklist.author_metadata_completed` until final author order, affiliations, emails, ORCID values when used, and corresponding author are author-confirmed. |
| Title page fields | The title page must include article title, author names, affiliations with full postal address, corresponding author, and any present or permanent address notes. | The final source package must replace anonymous author placeholders with target-system-visible title-page metadata before final upload. |
| Competing interests | The Elsevier declarations tool should be completed; the resulting Word document must be uploaded in `.doc` or `.docx` format, and authors with no competing interests should select the no-interest option. | Keep `publisher_declaration_files.competing_interest_declaration_file_verified` false until the Word file is generated and checked against the live submission field. |
| Funding sources | Funding sources and sponsor roles must be disclosed; when there is no specific grant, the no-funding sentence should be recorded. | Do not set `funding_statement_text_ready` until the final funding or no-funding wording is author-approved. |
| CRediT roles | The corresponding author must acknowledge co-author contributions using CRediT roles, with multiple roles allowed when applicable. | Do not set `contribution_statement_complete` until every listed author has author-approved contribution roles. |
| Vitae | Each author biography must be at most 100 words, editable, non-PDF, and paired with a passport-type photograph as a separate figure. | Keep biography and photograph paths outside anonymous preflight packages and record them only after author order is final. |

| Candidate | Officially verified constraints | Project implication |
| --- | --- | --- |
| Data & Knowledge Engineering | Elsevier lists 6.4 CiteScore and 2.6 Impact Factor, describes the journal as covering data engineering, knowledge engineering, and their interface, and uses single anonymized review. The author guide requests editable source files, supports LaTeX, limits the abstract to 250 words, requires 1--7 keywords, encourages 3--5 highlights with a maximum of 85 characters each, applies research-data deposit/linking or an explanatory data statement, requires a CRediT author contribution statement, requires a generative AI declaration when AI tools were used in manuscript preparation, and requests short author biographies with a maximum 100 words plus passport-type photographs; biography files must use an editable format and must not be PDF. | Keep DKE as the primary practical route. The current DKE package is a preflight artifact only; final upload still needs author metadata, target-template binding, a CRediT author contribution statement, author biographies and photographs, a generative AI declaration decision, and a real artifact URL or DOI. |
...
| DKE author biography and photograph request | Treat short biographies with a maximum 100 words, editable non-PDF biography files, and passport-type photographs as DKE-specific final-upload materials rather than anonymous preflight files. | Collect author-approved biographies and image files after author order is confirmed. |
...
7. Prepare short author biographies with a maximum 100 words, editable non-PDF biography files, and passport-type photographs if the live DKE/Elsevier route requires them.
