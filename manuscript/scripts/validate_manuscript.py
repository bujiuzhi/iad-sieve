...
FORMAL_PROCESS_TRACE_PATTERNS = [
    (re.compile(r"\b(?:Codex|Claude|ChatGPT|Copilot|Cursor)\b", re.IGNORECASE), "AI tool trace"),
...
def check_final_upload_information_request(request_text: str) -> list[str]:
    ...
    required_markers = [
        ...
        "`submission.target_journal`, `submission.article_type`, `corresponding_author`, `artifact_boundary`, `statements`, `funding`, `author_contributions`, `permissions`, and `generative_ai`",
        "`cover_letter.md` and live submission system",
        ...
        "Complete this cross-check before changing `ranking_confirmation_completed` to true",
        ...
        "separate image file and must use an image format",
        ".jpg",
        ".jpeg",
        ".png",
        ".tif",
        ...
        "biography_files or photograph_files counts that differ from the number of author rows",
        "Biography file path",
        "Photograph file path",
        "Photograph file must use image format",
        ...
        "Complete this cross-check before inserting an artifact URL or DOI into the final cover letter",
...
def check_data_code_availability_boundary(manuscript_text: str, supplementary_text: str = "") -> list[str]:
    ...
    required_main_markers = [
        ...
        "fixture rebuild verifier starts with the same CLI discovery check",
        "rejects missing public-source reconstruction commands",
        ...
        "schema contracts",
        "data-processing commands",
        r"\texttt{prepare-deepmatcher}",
        r"\texttt{prepare-scirepeval-proximity}",
        r"\texttt{fetch-openalex-works}",
        r"\texttt{prepare-openalex-weak-labels}",
        r"\texttt{build-iad-bench}",
        "schema-bound JSONL and summary outputs rather than raw-data copies",
...
def check_contribution_evidence_summary(manuscript_text: str) -> list[str]:
    ...
    required_markers = [
        ...
        "failure mode, benchmark contract, and merge mechanism",
        "false-merge risk rather than treated as merge confidence",
        "keeps gold identity labels, proxy agenda evidence, silver hard negatives, and human-review targets separate",
        "separate identity, agenda, and agenda-non-identity signals",
        "Open-v2 evidence snapshot evaluates these contributions under explicit row-scope boundaries",
        "clustering behavior, broad statistical ranking, and artifact-release audit claims remain outside the primary evidence",
        "targeted pair-level conclusion",
...
def check_scoring_merge_algorithm(manuscript_text: str, supplementary_text: str = "") -> list[str]:
    ...
    required_main_markers = [
        ...
        "identity, agenda, agenda-non-identity, and audit fields",
...
def check_result_interpretation_guardrails(manuscript_text: str, supplementary_text: str) -> list[str]:
    ...
    required_main_markers = [
        ...
        "row-family reading guide",
        "collapsed into one same-scope performance ranking",
        "FMR=0.001",
...
def check_target_journal_shortlist(shortlist_text: str) -> list[str]:
    ...
    required_markers = [
        ...
        "anonymous preflight package",
        "final-upload control",
        "Author list and order",
        "definitive at original submission",
        ...
        "Q2/B decision-making",
        "ISSN or eISSN",
        "subject category",
        "JCR quartile",
        "Chinese Academy of Sciences zone",
        "CCF class",
        "local institutional list",
        "publisher CiteScore, Impact Factor, aims-and-scope text, and this shortlist are screening evidence only",
...
def check_latex_build_scripts(build_script_text: str, elsevier_builder_text: str) -> list[str]:
    ...
    required_build_markers = [
        "TECTONIC_BUNDLE_DIR",
        "ORIGINAL_CWD",
        "export TECTONIC_BUNDLE_DIR",
        "--bundle",
        "scripts/diagnose_latex_environment.py",
        "--skip-logs",
        "run_tectonic",
...
def check_submission_system_checklist(checklist_text: str) -> list[str]:
    ...
    required_markers = [
        ...
        "Elsevier competing-interest declaration file",
        "Elsevier declarations tool",
        "publisher_declaration_files.competing_interest_declaration_file",
        "publisher_declaration_files.competing_interest_declaration_file_verified",
        ...
        "public artifact URL or DOI resolves to the release landing page",
        "artifact manifest `publication` object matches `submission_metadata.yml`",
        "manifest repository commit matches the final repository commit",
        ...
        "validate_submission_package.py --final-upload --artifact-dir /path/to/release` pass against the same release",
        ...
        "author-confirmed and recorded in `submission_metadata.yml`",
        "Final author identities, affiliations, corresponding author, CRediT contribution roles, funding statement, permissions statement, competing-interest declaration, generative AI declaration",
        "selected journal source and PDF files are rebuilt from the current source tree",
        "`repository_branch` equal to `main`",
        "A public artifact URL or DOI resolves",
        "artifact manifest `publication` object matches the final-upload metadata",
        "live submission system preview has been checked against the current title, abstract, keywords, highlights, uploaded files, declaration fields, and final package contents",
        "Q2/B ranking cross-check and artifact publication cross-check are complete",
...
def check_reviewer_readiness_audit(audit_text: str) -> list[str]:
    ...
    required_markers = [
        ...
        "cannot be resolved from the repository alone",
        "Local gates currently controlled by validators",
        "must still be rerun after source or package edits",
        ...
        "candidate-route official-source drift",
        ...
        "`anonymous preflight`",
        "`preflight cover letter`",
        "`submission-planning boundaries`",
        "`scope-fit note is preparatory`",
        "`must be replaced after author confirmation`",
        ...
        "does not download raw third-party data",
        ...
        "Anonymous Review-File Declaration Boundary Gate",
        "anonymous review-file declaration boundary",
        "does not assert a finalized competing-interest declaration",
...
def check_claim_interpretation_boundary(manuscript_text: str, supplementary_text: str = "") -> list[str]:
    ...
    required_main_markers = [
        ...
        "bounded pair-level mechanism result on Open-v2",
        "full numerical-audit claim",
        "same-scope prediction files",
        "source-input manifests",
        "processing-run logs",
        "statistical or component-causality claim",
        "bootstrap intervals",
        "accepted ablation variants",
        "deployment or broad method-ranking claim",
        "review-load records",
        "cluster-level artifacts",
        "abstract, conclusion, cover letter, and reviewer responses",
    ]
    required_supplement_markers = [
        r"\section{Claim Interpretation Boundary}",
...
def check_cover_letter(cover_letter_text: str) -> list[str]:
    ...
    required_markers = [
        ...
        "data and knowledge engineering venue",
        "database-oriented scholarly data integration",
        "knowledge engineering for scholarly records",
    ...
    required_markers_by_document = {
        "main abstract": (
            ...
            [
                ...
                "scope-bounded mechanism evidence",
                "same-scope comparative ranking",
                "RoBERTa pair-classifier row",
                "strong supervised comparator",
...
def check_submission_metadata(metadata_text: str) -> list[str]:
    ...
    required_markers = [
        ...
        "dke_official_guide_rechecked: \"2026-06-23\"",
        "information_systems_official_guide_rechecked: \"2026-06-23\"",
        "scientometrics_official_sources_rechecked: \"2026-06-23\"",
