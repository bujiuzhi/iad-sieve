...
| LaTeX/PDF build failures may be misread as manuscript-source failures. | Medium | `scripts/diagnose_latex_environment.py` records engine availability, `TECTONIC_BUNDLE_DIR`, inspected `build/logs/*.log`, and Tectonic/Rust runtime panic markers before source edits are attempted; PDF build scripts also run a pre-build diagnostic with `--skip-logs`. | Repair the local Tectonic engine or bundle before editing `main.tex` or rebuilding tracked PDFs. |
...
Outcome: pass for build-failure diagnostic coverage with the local Tectonic bundle path; source edits must continue to rebuild tracked PDFs through that bundle until the default engine path is repaired.

This gate checks whether a failed PDF build can be triaged without treating engine runtime failures as manuscript-source errors. `scripts/diagnose_latex_environment.py` records engine availability, `TECTONIC_BUNDLE_DIR`, inspected `build/logs/*.log`, and Tectonic/Rust runtime panic markers including `system-configuration`, `reqwest`, `Attempted to create a NULL object`, and `event loop thread panicked`. `build_latex_pdf.sh` and the standalone Elsevier/DKE preview builder now run the diagnostic with `--skip-logs` before invoking Tectonic, so a clean checkout can detect engine-level failures before build logs exist.
