...
This gate checks whether a DKE final-upload metadata packet can pass when `statements.research_data_statement` is empty, points only to the Git repository, or omits the public artifact URL or DOI. It cannot. The final-upload metadata validator rejects missing DKE research data statements and rejects DKE statements that do not contain the same artifact URL or DOI recorded under `artifact_boundary`.
