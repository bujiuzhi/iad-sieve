...
| The introduction contribution paragraph may be too compressed for first-screen review. | Medium | The contribution-evidence table already separates the problem contribution, benchmark contract, and IAD-Risk evidence boundary. | When the LaTeX/PDF build environment is available, revise the prose contribution paragraph to mirror that three-part structure and then rebuild the tracked PDFs before committing the source edit. |
| LaTeX/PDF build failures may be misread as manuscript-source failures. | Medium | `scripts/diagnose_latex_environment.py` records engine availability, `TECTONIC_BUNDLE_DIR`, inspected `build/logs/*.log`, Tectonic/Rust runtime panic markers, missing TeX resource errors, and Tectonic smoke test output excerpt before source edits are attempted; PDF build scripts also run a pre-build diagnostic with `--skip-logs`. | Repair the local Tectonic engine or bundle before editing `main.tex` or rebuilding tracked PDFs. |
...
This gate checks whether a failed PDF build can be triaged without treating engine runtime failures as manuscript-source errors. `scripts/diagnose_latex_environment.py` records engine availability, `TECTONIC_BUNDLE_DIR`, inspected `build/logs/*.log`, Tectonic/Rust runtime panic markers including `system-configuration`, `reqwest`, `Attempted to create a NULL object`, and `event loop thread panicked`, and missing TeX resource errors such as incomplete local bundle files. For non-panic smoke-test failures, it also prints a bounded Tectonic smoke test output excerpt so the first missing resource or command failure is visible. `build_latex_pdf.sh` and the standalone Elsevier/DKE preview builder now run the diagnostic with `--skip-logs` before invoking Tectonic, so a clean checkout can detect engine-level failures before build logs exist.
