...
This gate checks whether formal submission materials can pass when they avoid explicit Codex or ChatGPT strings but still contain non-obvious process notes. They cannot. The manuscript validator and submission-package validator now reject AI/tool traces including Copilot and Cursor, and process-note traces including assistant draft, assistant summary, prompt note, prompt log, system prompt, work summary, work log, edit log, change log, and revision log. These checks extend the existing Chinese process-note scan for terms such as 已修改, 修改记录, 工作记录, 工作总结, 本次修改, 本轮修改, 处理记录, and 变更记录.
