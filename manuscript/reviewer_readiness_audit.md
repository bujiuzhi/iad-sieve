...
| Round 5 | Official-source freshness audit | Pass for DKE preflight source; still blocked for target confirmation and ranking proof. | DKE guide rechecked on 2026-06-23 in `target_journal_shortlist.md` and `submission_metadata.yml`; target metrics treated as screening evidence only. | Re-open the selected journal guide and ranking/category source on submission day before changing final-upload or Q2/B wording. |
| Round 6 | Candidate-route source audit | Pass for current candidate-source planning records; still blocked for selected-route confirmation. | DKE, Information Systems, and Scientometrics official-source records were rechecked on 2026-06-23 in `target_journal_shortlist.md` and mirrored in `submission_metadata.yml`. | Treat secondary-route facts as route-planning evidence only until authors confirm the target journal, template, and ranking/category proof. |
| Round 7 | Final evidence-chain cross-check audit | Pass for checklist specificity; still blocked for real external evidence. | `final_upload_information_request.md` now requires Q2/B ranking cross-checks and artifact publication cross-checks before final-upload fields or claim wording are upgraded. | Complete the ranking evidence packet, public artifact release, manifest publication object, checksums, and final-upload package validation before inserting final claims or links. |
...
| Experimental strength | Conditional | Open-v2 evidence reports baseline HNFMR 0.790--0.999 and zero observed IAD-Risk HNFMR in the reported held-out hard-negative stratum. | Release same-scope prediction files, threshold logs, checksums, and result tables before relying on full numerical audit. |
...
| DKE biography and photograph lists may not cover every author. | Medium | Final-upload metadata validation now rejects DKE/Elsevier biography and photograph file counts that differ from the number of author rows. | Provide one editable biography file and one passport-type photograph file for each listed author before marking author identity materials verified. |
| DKE photograph paths may point to non-image files. | Medium | Final-upload metadata validation now rejects photograph file paths whose extensions are not `.jpg`, `.jpeg`, `.png`, `.tif`, or `.tiff`. | Record passport-type photograph paths as image files before marking author identity materials verified. |
| Git-only reviewers may see documented CLI commands that are no longer discoverable. | Medium | `verify_fixture_rebuild.py` now runs `python -m iad_sieve.cli --help` before rebuilding fixtures and rejects missing public-source reconstruction commands. | Keep `prepare-deepmatcher`, `prepare-scirepeval-proximity`, `fetch-openalex-works`, `prepare-openalex-weak-labels`, and `build-iad-bench` visible in CLI help before claiming Git-only reproducibility. |
| DKE research-data statements may mention only the Git repository or omit the required artifact/data-sharing boundary. | Medium | Final-upload metadata validation already rejects missing DKE research data statements, Git-only repository statements, statements that omit the artifact URL or DOI, and statements that omit the raw third-party data redistribution boundary. | Keep the DKE research data statement bound to the public artifact URL or DOI, the raw third-party data redistribution boundary, and the live submission-system field before final upload. |
| Public data-processing documentation may list commands without showing the source functions that implement them. | Medium | `docs/data-processing-pipeline.md` now maps each reconstruction command to its CLI handler, implementation module, main inputs, and main outputs; manuscript validation rejects missing command-map markers. | Keep the command-to-source map synchronized with `src/iad_sieve/cli.py` whenever data-processing commands are renamed, added, or removed. |
| The manuscript directory may blur tracked source files, tracked PDF previews, ignored local submission packages, external artifact releases, and local data/output directories. | Medium | `manuscript/README.md` and `manuscript/MANIFEST.md` now include a product-boundary table, and manuscript validation requires the table in both files. | Keep the product-boundary table synchronized whenever tracked PDF names, package paths, or artifact-release paths change. |
...
| Writing clarity self-review | Can a knowledgeable reader reproduce the method and understand each module's motivation? | pass | The method section defines identity, agenda, agenda-non-identity, risk score, thresholds, feature groups, and provenance-aware masking. | Recheck length, figure placement, and title-page formatting after target-journal template conversion. |
| Experimental strength self-review | Are the reported gains meaningful against strong baselines under a fair interpretation? | needs new experiment | Open-v2 evidence reports representation baseline HNFMR 0.790--0.999, RoBERTa FMR 0.001/HNFMR 0.0001, and zero observed IAD-Risk HNFMR in the reported held-out hard-negative stratum. | Release same-scope prediction files, threshold logs, checksums, and bootstrap intervals before claiming stronger comparative advantage. |
...
| If public-source APIs or dumps change after submission, can rerunning the commands still reproduce Open-v2? | Treat public-source commands as reconstruction code paths, not as frozen source snapshots. | Public-Source Rebuild, L2 public-source rebuild audit boundary, source_input_manifest, processing_run_log, and docs/data-processing-pipeline.md. | Do not promise identical Open-v2 numerical rows from live API calls or changed public dumps; exact result audit requires recorded acquisition dates or versions, input checksums, processing logs, and released derived artifacts. |
| If an artifact URL or DOI is added before upload, does that make the Open-v2 table fully reproducible? | Treat the link as access and provenance binding only until the release itself validates. | Data and Code Availability, artifact release README/manifest, final-upload source-control package binding, and submission package validation. | Do not upgrade full numerical audit, same-scope comparison, or stronger evidence claims until `validate_artifact_release.py` and `validate_submission_package.py --final-upload --artifact-dir` pass against the public release and matching commit. |
| Does IAD-Risk only move work from automatic merging to human review? | Treat the current result as a conservative safety filter unless same-artifact workload evidence is released. | Operational Complexity and Net Benefit, Selective Decision Coverage Boundary, and supplementary workload definitions. | Do not claim throughput reduction, staffing benefit, review-cost savings, or positive net benefit until automatic merge coverage, block rate, defer rate, review-required workload, capacity-normalized review load, and manual-review capacity are validated from the same prediction artifacts. |
| Why are exact identifier matching, title-normalization rules, or traditional entity-resolution systems not full main-result rows? | Use the baseline-inclusion rule: missing primary rows mean the current package lacks same-scope metric summaries, prediction files, threshold records, and checksums for those systems. | Baseline inclusion matrix, fairness-control matrix, and result artifact crosswalk. | Do not imply these omitted baselines were outperformed; add them as primary comparisons only after same-scope auditable artifacts are available. |
| Why is the manuscript positioned toward Data & Knowledge Engineering rather than Information Systems or Scientometrics? | Treat the DKE-style route as current scope-fit preparation, not final journal, ranking, or acceptance confirmation. | Journal Fit and Novelty Desk Check, DKE first-screen scope-fit gate, cover letter, highlights, and keywords. | Do not claim selected-journal confirmation, Q2/B completion, or final-upload readiness until the author-approved target, ranking evidence, template requirements, and live submission-system fields are recorded. |
...
| The mechanism may not be causal without ablations. | `do_not_answer_as_claim` | Use only the current mechanism-consistent wording. | Do not claim completed component causality before no-risk-gate, no-ANI-head, single-space, no-cannot-link, and post-hoc-threshold artifacts are released. |
...
Outcome: pass for mechanism-evidence table-density reduction; blocked for full causal-mechanism claims until ablations removing risk gating, agenda-non-identity supervision, and single-space separation are released with checksum-fixed artifacts.
...
The reviewer-facing boundary is feature-head clarity without main-text table overload. The validator still requires the supplementary feature and head specification table, including Identity head, Agenda head, agenda-non-identity risk head, Risk gate, Audit metadata, the direct statement that metadata fields are retained for auditing but are not used as model features, and the rule that audit metadata is not a training feature.
...
This gate checks whether the Related Work section makes the novelty boundary visible without relying on a citation list alone. The main manuscript now states that end-to-end entity resolution, neural matching, scientific representation learning, and open metadata benchmarks assign different decision roles to relatedness. It also frames the data-integration gap as recording provenance, label strength, and relation evidence roles before a merge policy treats relatedness as positive, negative, or deferred evidence. IAD-Risk is positioned more narrowly: relatedness is interpreted only after the pair schema records whether the signal is identity evidence, agenda evidence, or agenda-non-identity stress evidence. The final positioning now uses compact paragraphs so the novelty and boundary read as one reviewer-facing argument.
...
This gate checks whether the first-screen submission materials state a concrete data and knowledge engineering scope fit instead of only naming broad adjacent areas. The abstract now opens from scholarly data integration, and the introduction opens from scholarly data integration and knowledge engineering systems before narrowing to work deduplication. The cover letter limits the DKE-style editorial-screen claim to database-oriented scholarly data integration, knowledge engineering for scholarly records, entity matching, benchmark construction, and reproducible data-processing contracts. The highlights also frame identity-agenda confusion as a data/knowledge-engineering merge risk.
...
This gate checks whether the first-screen abstract and preflight cover letter could report zero observed HNFMR without the ordinary FMR boundary or hard-negative stratum boundary. The abstract and cover letter now state that IAD-Risk reports same-work F1=0.980 and zero observed HNFMR in the held-out hard-negative stratum, with ordinary FMR still reported separately as 0.001.

The gate also checks first-screen method-evidence alignment. The abstract now motivates identity-agenda confusion through semantic relatedness converted into merge evidence, states that a single match score does not reveal whether evidence reflects identity or agenda relatedness, and states that the RoBERTa pair-classifier row remains a strong supervised comparator rather than broad-superiority evidence. The cover letter uses the corresponding single-score matching formulation and states that IAD-Risk exposes identity, agenda, and agenda-non-identity signals separately, rather than making a broad pair-classifier failure claim. This keeps first-screen text aligned with the Open-v2 table, where RoBERTa is reported as a strong supervised comparator rather than as the main high-HNFMR failure row.
...
This gate checks whether the submission highlights could still expose the zero-HNFMR overread after the abstract and cover letter were corrected. The quantitative highlight now states: "Open-v2 held-out hard-negative scope: zero observed HNFMR; ordinary FMR=0.001." The method highlight avoids the unexplained `ANI` abbreviation and instead states that IAD-Risk separates identity, agenda, and non-identity risk. The boundary highlight states that cluster-level claims require artifact-backed audits and that the highlights make no ranking or statistical claim.
...
This gate records a reviewer-facing writing risk in the introduction contribution paragraph and the first related-work positioning bridge. The introduction now states the root challenge as a decision-semantics gap: scientific representations are useful for relatedness, but their objective does not say whether a close scholarly pair should be merged, blocked, or deferred. The contribution-evidence table already uses the correct three-part structure: identity-agenda confusion as the problem and metric contribution, IAD-Bench as the provenance-aware benchmark contract, and IAD-Risk as the risk-aware merge mechanism with Open-v2 evidence and explicit claim boundaries. The prose contribution paragraph now mirrors that structure with one clear sentence per contribution and a final sentence that keeps clustering, statistical-ranking, and artifact-audit claims outside the current primary evidence. The Related Work bridge now frames closest-work positioning as a novelty audit across end-to-end entity resolution, neural entity matching, scientific document representations, and open scholarly metadata benchmarks rather than as another result table.
...
Outcome: pass for DKE author identity material cardinality coverage; blocked for final upload until each listed author has one editable biography file and one passport-type photograph file recorded.

This gate checks whether a DKE/Elsevier final-upload metadata packet can pass with fewer biography or photograph files than listed authors. It cannot. The metadata validator compares the number of parsed `authors` rows with `author_identity_materials.biography_files` and `author_identity_materials.photograph_files` whenever the DKE/Elsevier route requires author identity materials, and it rejects mismatches with `author biography file count must match author count` or `author photograph file count must match author count`.

The reviewer-facing boundary is author-material completeness, not scientific evidence. This gate prevents a partial author-material packet from passing only because at least one biography file and one photograph file were present. It does not place identity-bearing author materials into the anonymous preflight package and does not replace live submission-system verification.

## Readiness Gate 133: DKE Photograph File Format Gate

Outcome: pass for DKE photograph file-format coverage; blocked for final upload until every recorded photograph path uses an image file extension.

This gate checks whether a DKE/Elsevier final-upload metadata packet can pass when `author_identity_materials.photograph_files` points to a PDF, Word document, or extensionless placeholder rather than a photograph image. It cannot. The metadata validator now checks each photograph path and rejects files without an image extension or files whose extension is not `.jpg`, `.jpeg`, `.png`, `.tif`, or `.tiff`.

The reviewer-facing boundary is publisher upload hygiene, not scientific evidence. This gate does not create author-approved photographs and does not put identity-bearing files into the anonymous preflight package. It prevents a final-upload metadata packet from treating a non-image path as a passport-type photograph.

## Readiness Gate 134: Git-Only CLI Discovery and Fixture Rebuild Gate

Outcome: pass for CLI discovery coverage; blocked for Git-only reproducibility claims if the installable CLI help output does not expose the public-source reconstruction commands.

This gate checks whether the repository can still support the Data and Code Availability claim that reviewers can discover the data-processing command chain before running fixture or artifact checks. `verify_fixture_rebuild.py` now starts with `python -m iad_sieve.cli --help` and rejects help output that omits `prepare-deepmatcher`, `prepare-scirepeval-proximity`, `fetch-openalex-works`, `prepare-openalex-weak-labels`, or `build-iad-bench`.

The reviewer-facing boundary is command-chain discoverability, not full numerical reproduction. This gate confirms that the Git repository exposes the public-source reconstruction entry points used by the manuscript, but it does not download raw third-party data, reproduce Open-v2 numerical rows, or replace the L2/L3 artifact release.

## Readiness Gate 135: DKE Research Data Statement Gate

Outcome: pass for DKE research-data statement validator coverage; blocked for final upload until the selected DKE route records a journal-system research data statement with a real artifact URL or DOI.

This gate checks whether a DKE final-upload metadata packet can pass when `statements.research_data_statement` is empty, points only to the Git repository, omits the public artifact URL or DOI, or omits the raw third-party data redistribution boundary. It cannot. The final-upload metadata validator rejects missing DKE research data statements, Git-only repository statements, statements that do not contain the same artifact URL or DOI recorded under `artifact_boundary`, and statements that do not preserve the raw third-party data redistribution boundary.

The reviewer-facing boundary is journal-system data-statement compliance, not full numerical reproduction. This gate aligns the source package with the DKE research-data expectation that data be deposited, linked, or explained in the submission statement. It does not publish the external artifact, verify public access, or reproduce the Open-v2 numerical table without the L2/L3 artifact release.

## Readiness Gate 136: Data-Processing Command Implementation Map Gate

Outcome: pass for public data-processing command-map coverage; must be rerun after any CLI, data-adapter, or public documentation edit.

This gate checks whether the public data-processing documentation can claim that the repository contains reproducible processing code while listing only command names. It cannot. The manuscript validator now requires `docs/data-processing-pipeline.md` to include a command-to-source map that binds `prepare-deepmatcher`, `prepare-scirepeval-proximity`, `fetch-openalex-works`, `prepare-openalex-weak-labels`, and `build-iad-bench` to their CLI handler functions, implementation modules, main inputs, and main outputs.

The reviewer-facing boundary is code traceability, not full numerical reproduction. This gate helps a Git-only reviewer locate the implementation behind each public-source reconstruction command, but it does not provide raw third-party files, freeze API responses, or replace the artifact manifest, processing log, and checksum chain required for L2/L3 audit.

## Readiness Gate 137: Manuscript Product Boundary Table Gate

Outcome: pass for manuscript product-boundary table coverage; must be rerun after any manuscript package, PDF, build, or artifact-release path change.

This gate checks whether the manuscript directory can satisfy the single-directory product requirement while leaving product classes implicit. It cannot. The manuscript validator now requires `manuscript/README.md` and `manuscript/MANIFEST.md` to include a product-boundary table that separates tracked journal sources, tracked PDF previews, ignored local submission packages, external result artifacts, and ignored local data/output directories.

The reviewer-facing boundary is directory hygiene, not final-upload readiness. This gate confirms that the repository exposes a single tracked manuscript-product directory and explains which generated or external products stay outside Git; it does not create the final author-confirmed upload package, publish the result artifact, or replace live submission-system verification.

## Readiness Gate 138: Main-Result Strong-Baseline Interpretation Gate

Outcome: pass for RoBERTa strong-baseline interpretation coverage; must be rerun after any result-table, result-interpretation, conclusion, or cover-letter edit.

This gate checks whether the main result section or cover letter could be read as dismissing the RoBERTa pair classifier or as claiming broad superiority from mixed row scopes. It cannot. The main result interpretation states that RoBERTa pair classification is a strong baseline that can already suppress many unsafe merges under the full Open-v2 scope, while IAD-Risk should be read as an auditable relation-role design rather than a broad superiority claim. The pre-submission cover letter now keeps the same boundary by treating RoBERTa pair classification as a strong supervised baseline and describing IAD-Risk as an auditable relation-role design rather than a broad superiority claim.

The reviewer-facing boundary is strong-baseline interpretation, not new empirical evidence. The gate preserves the current claim that identity, agenda, and agenda-non-identity signals are exposed before a fixed risk gate decides whether a related pair may merge, block, or defer. It does not convert held-out IAD-Risk rows and full-scope RoBERTa rows into a same-scope comparative ranking, and it does not remove the need for same-scope prediction files, threshold logs, bootstrap intervals, and checksums before any stronger ranking claim.

## Readiness Gate 139: Abstract Strong-Baseline First-Screen Gate

Outcome: pass for abstract strong-baseline first-screen alignment; must be rerun after any abstract, result-table, cover-letter, or conclusion edit.

This gate checks whether the abstract could still present the Open-v2 evidence as a two-way contrast between weak representation baselines and IAD-Risk while hiding the strong supervised RoBERTa comparator. It cannot. The abstract now states that the RoBERTa pair-classifier row remains a strong supervised comparator and is not broad-superiority evidence, while the result section and cover letter keep the same relation-role design boundary.

The reviewer-facing boundary is abstract first-screen fairness, not new empirical evidence. The added sentence prevents editors from reading the abstract as a broad model-ranking claim, but it does not create same-scope comparison, statistical superiority, or threshold-stability evidence. Stronger comparison still requires same-scope prediction files, threshold logs, bootstrap intervals, and checksums.
...
## Readiness Gate 141: Reviewer Workload-Transfer Response Gate

Outcome: pass for reviewer workload-transfer response coverage; blocked for throughput or cost-saving claims until same-artifact workload evidence is released.

This gate checks whether the reviewer-response matrix can answer a likely method-design objection: IAD-Risk may reduce false merges by moving too many pairs into block or defer states. The matrix now states that the present evidence should be read as a conservative safety filter unless the same prediction artifacts report automatic merge coverage, block rate, defer rate, review-required workload, capacity-normalized review load, and manual-review capacity.

The reviewer-facing boundary is response discipline, not deployment evidence. The current manuscript can answer why the method is safety-oriented and auditable, but it cannot claim throughput reduction, staffing benefit, review-cost savings, or positive operational net benefit until the workload fields validate from the same prediction artifacts that support FMR and HNFMR.

## Readiness Gate 142: Rule-Based Baseline Omission Response Gate

Outcome: pass for rule-based and traditional-baseline omission response coverage; blocked for broad baseline superiority claims until same-scope auditable artifacts are released.

This gate checks whether the reviewer-response matrix can answer why exact identifier matching, title-normalization rules, or traditional entity-resolution systems do not appear as full primary rows in the Open-v2 result table. The matrix now states that omission is an artifact-readiness boundary: a primary row requires same-scope metric summaries, prediction files, threshold records, and checksums.

The reviewer-facing boundary is comparison fairness, not stronger empirical evidence. The manuscript can say that these baseline families are documented and not used as unsupported outperformance claims, but it cannot imply they were beaten until same-scope auditable artifacts are available.

## Readiness Gate 143: Target-Route Reviewer Response Gate

Outcome: pass for target-route reviewer response coverage; blocked for selected-journal or Q2/B completion claims until author-confirmed target and ranking evidence are recorded.

This gate checks whether the reviewer-response matrix can answer why the current package is positioned toward a Data & Knowledge Engineering style route rather than Information Systems or Scientometrics. The matrix now states that the DKE-style route is scope-fit preparation based on the current data-integration, knowledge-engineering, entity-matching, benchmark-contract, and reproducible-processing framing, not final target-journal confirmation.

The reviewer-facing boundary is target-route discipline, not journal acceptance evidence. The manuscript can explain why DKE is the conservative route for the present framing, but it cannot claim selected-journal confirmation, Q2/B completion, or final-upload readiness until the author-approved target, ranking evidence, template requirements, and live submission-system fields are recorded.

## Readiness Gate 144: Artifact-Link Claim-Upgrade Response Gate

Outcome: pass for artifact-link claim-upgrade response coverage; blocked for full numerical audit or stronger comparison claims until the public artifact release and matching final-upload package validate.

This gate checks whether the reviewer-response matrix could treat adding a public artifact URL or DOI as sufficient proof that the Open-v2 table is fully reproducible. It cannot. The matrix now states that an artifact URL or DOI is only an access and provenance binding until `validate_artifact_release.py` and `validate_submission_package.py --final-upload --artifact-dir` pass against the public release, its checksum file, its publication metadata, and the matching repository commit.

The reviewer-facing boundary is artifact-link discipline, not evidence upgrade. The manuscript may cite a real URL or DOI after the release is public and metadata-aligned, but it must not upgrade full numerical audit, same-scope comparison, threshold stability, statistical superiority, component causality, or human-validation claims unless the released artifacts contain the required prediction files, logs, manifests, checksums, and protocol outputs.

## Readiness Gate 145: Public-Source Temporal Drift Response Gate

Outcome: pass for public-source temporal drift response coverage; blocked for exact result reproduction claims unless acquisition records, input checksums, processing logs, and released derived artifacts are available.

This gate checks whether the reviewer-response matrix can answer a likely reproducibility objection: public APIs, dumps, and metadata exports can change after submission. The matrix now states that public-source commands are reconstruction code paths, not frozen source snapshots. Re-running `fetch-openalex-works`, `prepare-openalex-weak-labels`, or other public-source commands against changed inputs may rebuild the same artifact classes but cannot be promised to reproduce the identical Open-v2 numerical rows.

The reviewer-facing boundary is temporal-source discipline, not a weaker reproducibility standard. Git should keep the code, CLI entry points, schemas, fixtures, and documentation; exact result audit additionally needs `source_input_manifest` acquisition dates or versions, input checksums, `processing_run_log` entries, derived output checksums, and released result artifacts. Without that chain, the manuscript may claim code-path reproducibility but must not claim fixed-source or fixed-number reproducibility from live public-source calls.

## Readiness Gate 146: Final Cover-Letter Sentence Map Gate

Outcome: pass for final cover-letter sentence-map specificity; blocked for final upload until target journal, corresponding author, artifact URL or DOI, and declaration text are author-confirmed.

This gate checks whether the final-upload information request gives authors a concrete sentence map rather than only a checklist. The request now maps the greeting line, submission sentence, scope-fit sentence, evidence-boundary sentence, artifact sentence, declaration sentence, and signature line to `submission.target_journal`, `submission.article_type`, `target_preparation`, `artifact_boundary.artifact_release_url` or `artifact_boundary.artifact_release_doi`, declaration fields, and `corresponding_author.name`. The final-upload cover-letter validator now also requires the evidence-boundary sentence to preserve Open-v2 scope-bounded mechanism evidence, the same-scope comparative-ranking boundary, and the absence of confidence-interval, statistical-significance, or model-ranking claims unless validated artifacts support them.

The reviewer-facing boundary is final-upload assembly discipline, not new scientific evidence. The map keeps the anonymous pre-submission cover letter separate from the final target-specific cover letter and prevents copying `Dear Editor`, `Anonymous Authors`, Git-only preflight wording, or unsupported Q2/B-complete language into the live submission system. It also prevents artifact-link wording that upgrades the Open-v2 evidence into interval-supported model ranking.

## Readiness Gate 147: Data-Processing Git-Only Reproduction Boundary Gate

Outcome: pass for public data-processing reproduction-boundary coverage; blocked for Open-v2/Open-v3 numerical audit until L2 public-source rebuild or L3 result audit artifacts are released.

This gate checks whether `docs/data-processing-pipeline.md` could be read as saying that Git alone reproduces the manuscript's main numerical rows. It cannot. The public-release checker now requires the data-processing document to include an explicit reviewer-facing boundary: Git-only review confirms L0 code check and L1 fixture rebuild, but cannot verify Open-v2/Open-v3 main-table numbers. Full result audit requires L2 public-source rebuild or L3 result audit evidence, including `configs/source_input_manifest.json`, `logs/processing_run_log.jsonl`, `checksums.sha256`, and the external artifact release.

The reviewer-facing boundary is data-processing documentation discipline, not new numerical evidence. This gate aligns the public documentation with the manuscript's Data and Code Availability statement, supplementary reproduction levels, and cover-letter Git-only boundary. It does not provide raw third-party data, freeze public API responses, publish prediction files, or validate the missing L2/L3 artifacts. Future data-processing documentation edits must keep Git-only reproduction from being presented as full numerical audit evidence.

## Readiness Gate 148: Candidate Official-Source Planning Gate

Outcome: pass for candidate-route official-source planning coverage; blocked for selected-target, ranking/category, template, and live-system claims until author-confirmed evidence is recorded.

This gate checks whether the updated Information Systems and Scientometrics official-source records could be mistaken for target-journal selection or Q2/B proof. They cannot. The target-journal shortlist now records DKE, Information Systems, and Scientometrics official-source checks on 2026-06-23, and `submission_metadata.yml` mirrors the secondary candidate source dates and URLs.

The reviewer-facing boundary is route-planning freshness, not target selection or ranking evidence. Current publisher metrics, scope statements, review models, and formatting requirements can guide the next manuscript route decision, but they do not replace author confirmation, institutional ranking/category evidence, selected template binding, artifact publication, or live submission-system verification.

## Readiness Gate 149: Final Evidence-Chain Cross-Check Gate

Outcome: pass for final-upload evidence-chain checklist specificity; blocked for Q2/B completion, artifact-link insertion, and final-upload claims until real external evidence is supplied and validated.

This gate checks whether the final-upload information request or submission-system checklist can still let authors treat a ranking label, publisher metric, artifact URL, or DOI as a standalone proof object. It cannot. The request and checklist now separate Q2/B ranking cross-checks from artifact publication cross-checks. The ranking cross-check requires the selected journal name to match `submission.target_journal`, the selected ISSN or eISSN to match the ranking source lookup, the category and reported value to appear in the evidence export or screenshot, the ranking-source access date to be no later than the final upload date, responsible-author confirmation, and first-screen materials that avoid Q2/B-complete wording unless the packet is complete and validated.

The artifact publication cross-check requires the public artifact URL or DOI to resolve to the release landing page, the artifact manifest `publication` object to match `submission_metadata.yml`, the manifest repository commit to match the final repository commit, `checksums.sha256` to cover result files, source manifest, and processing logs, `configs/source_input_manifest.json` and `logs/processing_run_log.jsonl` to be included in the public release, and both artifact-release validation and final-upload package validation to pass against the same release.

The reviewer-facing boundary is evidence-chain completeness, not scientific evidence strengthening. This gate makes final-upload dependencies auditable before authors insert links or route labels into the live system, but it does not create the ranking evidence, public artifact release, checksums, source manifest, processing logs, author confirmation, or live-system preview. Q2/B-complete wording, artifact-backed numerical audit claims, and final-upload readiness remain blocked until the external evidence exists and validates.
...
18. Method threshold notation uses `\tau_n` for the agenda-non-identity risk-head threshold and contains no ambiguous `\tau_a` gate notation in the submitted method text.
...
20. The pre-submission cover letter preserves the Git-only review boundary and public-source temporal boundary and does not imply that the Open-v2 numerical table is reproducible from Git alone or from changed public-source calls before the artifact release URL or DOI, checksum-bound prediction files, threshold logs, source manifests, processing logs, acquisition records, input checksums, and released derived artifacts are available.
...
25. DKE/Elsevier final-upload metadata records one editable biography file and one passport-type photograph file per listed author, and both `biography_files` and `photograph_files` counts match the number of `authors` rows.
26. DKE/Elsevier final-upload metadata records passport-type photograph paths with image extensions `.jpg`, `.jpeg`, `.png`, `.tif`, or `.tiff`; no PDF, Word, or extensionless photograph paths are accepted.
27. Git-only fixture rebuild validation starts with `python -m iad_sieve.cli --help` and confirms `prepare-deepmatcher`, `prepare-scirepeval-proximity`, `fetch-openalex-works`, `prepare-openalex-weak-labels`, and `build-iad-bench` remain discoverable.
28. The DKE/Elsevier final-upload metadata includes a `statements.research_data_statement` that names the public artifact URL or DOI, preserves the raw third-party data redistribution boundary, and matches the live submission-system research data statement field.
29. `docs/data-processing-pipeline.md` keeps a command-to-source map that binds the public-source reconstruction commands to their CLI handler functions, implementation modules, main inputs, and main outputs.
30. `manuscript/README.md` and `manuscript/MANIFEST.md` keep a product-boundary table that separates tracked journal sources, tracked PDF previews, ignored local submission packages, external result artifacts, and ignored local data/output directories.
31. Main-result, conclusion, cover-letter, and reviewer-response wording keep RoBERTa pair classification as a strong supervised baseline while preserving the IAD-Risk contribution as an auditable relation-role design rather than a broad superiority claim or same-scope comparative ranking.
32. The abstract states that the RoBERTa pair-classifier row remains a strong supervised comparator and not broad-superiority evidence before reporting IAD-Risk held-out rows.
33. The abstract keeps at least a 15-word DKE/Elsevier word-budget buffer below the 250-word front-matter limit after any abstract, keyword, template, or Elsevier source edit.
34. Reviewer-response wording treats IAD-Risk as a conservative safety filter unless same-artifact workload evidence validates automatic merge coverage, defer rate, review-required workload, capacity-normalized review load, and manual-review capacity.
35. Reviewer-response wording treats omitted exact identifier, title-normalization, and traditional entity-resolution baselines as artifact-readiness gaps, not as baselines that IAD-Risk has already outperformed.
36. Reviewer-response wording treats the DKE-style route as scope-fit preparation and does not claim selected-journal confirmation, Q2/B completion, or final-upload readiness before author-approved target and ranking evidence are recorded.
37. Reviewer-response wording treats an artifact URL or DOI as access/provenance binding only; it does not upgrade full numerical audit, same-scope comparison, or stronger evidence claims before the public release and matching final-upload package validate.
38. Reviewer-response wording treats public-source commands as reconstruction code paths rather than frozen source snapshots; exact Open-v2 numerical reproduction requires recorded acquisition dates or versions, input checksums, processing logs, and released derived artifacts.
39. Final cover-letter assembly uses the sentence map and keeps the anonymous pre-submission cover letter separate from the final target-specific cover letter.
40. `docs/data-processing-pipeline.md` states that Git-only review confirms L0 code check and L1 fixture rebuild only; Open-v2/Open-v3 numerical audit requires L2 public-source rebuild or L3 result audit artifacts with `source_input_manifest`, `processing_run_log`, `checksums.sha256`, and an external artifact release.
41. `final_upload_information_request.md` records both the Q2/B ranking cross-check and artifact publication cross-check, and `submission_system_checklist.md` mirrors both checks for live-system upload; final-upload package validation is not run as a pass condition until those external evidence chains are complete.
