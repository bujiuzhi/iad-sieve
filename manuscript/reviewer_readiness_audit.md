...
| LaTeX/PDF build failures may be misread as manuscript-source failures. | Medium | `scripts/diagnose_latex_environment.py` records engine availability, `TECTONIC_BUNDLE_DIR`, inspected `build/logs/*.log`, and Tectonic/Rust runtime panic markers before source edits are attempted. | Repair the local Tectonic engine or bundle before editing `main.tex` or rebuilding tracked PDFs. |
