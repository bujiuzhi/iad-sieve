...
This gate checks whether formal submission materials can pass when they avoid explicit named-tool strings but still contain non-obvious process notes. They cannot. The manuscript validator and submission-package validator reject named assistant-tool traces including Copilot and Cursor, and process-note traces including assistant draft, assistant summary, prompt note, prompt log, system prompt, work summary, work log, edit log, change log, revision log, revision note, change record, implementation note, 本轮处理, and 调整记录. These checks extend the existing multilingual process-note scan used by the formal-material validators.
...
15. Formal manuscript files, cover-letter text, metadata, and generated packages pass the expanded process-trace scan without assistant draft, prompt note, system prompt, work summary, edit log, Copilot, or Cursor traces.
