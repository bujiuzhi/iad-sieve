...
Table~\ref{tab:result-interpretation-guardrails} states how the Open-v2 table should be read before external artifact release and same-scope prediction files are available. The main result table includes Scope type labels to distinguish full available Open-v2 scope rows from held-out Open-v2 test scope rows. Scope labels prevent leaderboard reading when row families are supported by different available prediction scopes. The intended interpretation is mechanism-oriented; the table labels this as a Mechanism-supported reading rather than a leaderboard-oriented reading.
...
\midrule
Representation baselines & The representation rows test false-merge exposure when one-space scientific relatedness is used as merge evidence. & They do not define the best possible representation-threshold system under a fully tuned, same-scope leaderboard. \\
...
Whole table & The table gives a mechanism-supported reading that semantic relatedness alone can be unsafe and risk gating can block hard-negative merges in the reported setting. & The table is not a same-scope leaderboard and is not evidence of threshold stability or zero risk. \\
\bottomrule
\end{tabular}
\end{table}
