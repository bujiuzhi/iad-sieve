...
Scientific document representation methods learn embeddings that capture citation, topical, and semantic relatedness. SPECTER uses citation-informed supervision to learn document-level representations \citep{cohan2020specter}, and SciNCL further improves scientific representations with neighborhood contrastive learning \citep{ostendorff2022scincl}. These methods are appropriate baselines because they capture scientific relatedness, yet their strength is also a risk for deduplication: a representation that intentionally brings related papers close can over-connect papers that share an agenda but are not the same work.
