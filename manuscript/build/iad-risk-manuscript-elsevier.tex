...
\section{Reviewer-Facing Claim Checklist}

Table~\ref{tab:reviewer-facing-claim-checklist} summarizes the claim boundary that should guide interpretation of the manuscript. The checklist is included to make the main evidence location, supported wording, and boundary before stronger wording explicit for readers. It also separates manuscript-supported conclusions from claims that require artifact-backed ablations, a manual-validation slice, or broader source-heldout validation.

\begin{table}[H]
\caption{Reviewer-facing claim checklist. Each row maps a common review dimension to the present evidence and the wording boundary that should not be crossed without additional artifacts.}
\label{tab:reviewer-facing-claim-checklist}
\centering
\footnotesize
\setlength{\tabcolsep}{3pt}
