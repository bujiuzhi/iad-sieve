...
The project repository contains version-controlled source code, benchmark construction scripts, public-release checks, and manuscript sources. The installable package source is under \path{src/iad_sieve}; \path{pyproject.toml} declares the package root and exposes \texttt{iad-sieve = iad\_sieve.cli:main}. The command \texttt{python -m iad\_sieve.cli --help} verifies command discovery before fixture rebuild or artifact commands are run.

The repository includes source code and CLI entry points, small public fixtures, fixture tests, schema contracts, data-processing commands, and artifact-release guidance. The public-source reconstruction chain is documented through \texttt{prepare-deepmatcher}, \texttt{prepare-scirepeval-proximity}, \texttt{fetch-openalex-works}, \texttt{prepare-openalex-weak-labels}, and \texttt{build-iad-bench}; these commands write schema-bound JSONL and summary outputs rather than raw-data copies. This data-processing path is version-controlled in Git, but raw third-party source files, full prediction files, model checkpoints, and derived evaluation artifacts remain outside Git unless released through an external artifact package.

The full data and code availability boundary table is reported in the supplementary material. The boundary distinguishes L0/L1 code-level reproduction from L2/L3 result-level audit: reviewers with only the Git repository can run the CLI help check, fixture rebuild validation via \texttt{python} \path{manuscript/scripts/verify_fixture_rebuild.py}, the public-release audit via \texttt{python} \path{scripts/check_public_release.py}, and the manuscript validation scripts. These checks verify executable contracts, fixture behavior, public-release boundaries, and manuscript builds, but they do not reproduce the Open-v2 numerical table, model predictions, threshold choices, or bootstrap intervals.

Full numerical reproduction requires public-source rebuilds or released artifacts. An L2 rebuild should record \path{source_input_manifest} and \path{processing_run_log} so that public input provenance, processing commands, code commits, environments, seeds, output paths, exit status, and checksums can be audited. Raw third-party data are not redistributed because the original sources have their own access conditions and licenses; therefore an external release should redistribute derived tables, predictions, logs, manifests, and checksums rather than raw provider files unless the original provider terms explicitly allow redistribution, with commit identifiers recorded in the release metadata and each source entry recording its license boundary. The repository therefore avoids redistributing raw data or full experiment outputs in Git.

An external artifact package should be treated as result evidence only after artifact-level and manuscript-package validation pass. Before copying generated result files, \path{manuscript/scripts/populate_artifact_release.py} with \texttt{--artifact-dir} \path{/path/to/release}, \texttt{--source-dir} \path{/path/to/source-artifacts}, and \texttt{--preflight-only} checks that the source artifact directory contains the files required by the release manifest without writing release files. The source preflight can detect missing tables, predictions, reports, configurations, or logs early, but it is not itself result evidence. The release is then checked with \path{manuscript/scripts/validate_artifact_release.py} \texttt{--artifact-dir /path/to/release}, and the final upload package is checked with \path{manuscript/scripts/validate_submission_package.py} \texttt{--final-upload --artifact-dir /path/to/release}. These checks bind the artifact manifest, submission metadata, source-control commit, and final package to the same source commit; if validation fails, the artifact package should not be used to support the Open-v2 numerical table or any stronger claim.
