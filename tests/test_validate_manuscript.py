...
def test_check_final_upload_information_request_rejects_missing_cover_letter_replacement_packet() -> None:
    ...
    assert any("Final cover letter sentence map" in error for error in errors)
    assert any("Evidence-boundary sentence" in error for error in errors)
...
def test_check_highlights_rejects_zero_hnfmr_without_hard_negative_scope() -> None:
    """验证 zero observed HNFMR 的 highlight 必须限定 hard-negative 范围。"""
    ...
    assert any("hard-negative scope boundary" in error for error in errors)


def test_check_highlights_rejects_rounded_zero_hnfmr_wording() -> None:
    """验证 highlights 不接受 HNFMR=0.000 作为首屏零值表述。"""
...
def test_check_method_feature_contract_accepts_supplementary_table() -> None:
    ...
    manuscript_text = "\n".join(
        [
            ...
            "The agenda-non-identity risk head uses different-identifier conflicts.",
...
def test_check_method_pipeline_figure_accepts_complete_figure() -> None:
    ...
    manuscript_text = "\n".join(
        [
            ...
            "Work, agenda,\\\\agenda-non-\\\\identity heads",
            "Risk-aware\\\\merge gate",
            "Merge, block,\\\\or defer",
            r"\caption{IAD-Risk pipeline. The framework predicts work, agenda, and agenda-non-identity heads.}",
...
def test_check_scoring_merge_algorithm_accepts_complete_contract() -> None:
    ...
    manuscript_text = "\n".join(
        [
            ...
            "IAD-Risk first builds identity, agenda, agenda-non-identity, and audit fields.",
...
def test_check_target_journal_shortlist_accepts_complete_shortlist() -> None:
    ...
    shortlist_text = "\n".join(
        [
            ...
            "All candidate-route publisher-page facts in this shortlist were rechecked on 2026-06-23.",
            "Information Systems and Scientometrics entries are current official-source planning records.",
            ...
            "serious experimentation expectations for systems papers.",
            "Scientometrics route.",
            "single-blind review.",
            "150--250 word abstract.",
            "4--6 keywords.",
            "large-language-model documentation rules.",
...
def test_check_target_journal_shortlist_rejects_missing_current_source_snapshot() -> None:
    ...
    assert any("DKE official source snapshot date: 2026-06-23" in error for error in errors)
...
def test_check_related_work_positioning_accepts_main_text_and_supplementary_matrix() -> None:
    ...
    manuscript_text = "\n".join(
        [
            ...
            "provenance, label strength, and relation evidence roles",
            "merge policy treats relatedness as positive, negative, or deferred evidence",
...
def test_check_reviewer_readiness_audit_accepts_complete_audit() -> None:
    ...
    audit_text = "\n".join(
        [
            ...
            "repeated skeptical-review passes",
            "quality-control record, not a manuscript-upload file",
            "pass status from being interpreted as acceptance readiness",
            "Round 1",
            "Desk-screen claim audit",
            "bounded first-screen claims",
            "Round 2",
            "Reproducibility and package audit",
            "Pass for Git-only review; not full numerical audit",
            "`verify_fixture_rebuild.py`, `check_public_release.py`, strict manuscript validation, submission-package validation, and DKE preflight validation",
            "L2/L3 source or artifact release",
            "Round 3",
            "Final-upload boundary audit",
            "Blocked for final upload",
            "source-control package binding, and final cover-letter sentence map",
            "Round 4",
            "Q2/B upgrade audit",
            ...
            "Does IAD-Risk only move work from automatic merging to human review?",
            "conservative safety filter",
            "throughput reduction",
            "staffing benefit",
            "review-cost savings",
            "positive net benefit",
            "Why are exact identifier matching, title-normalization rules, or traditional entity-resolution systems not full main-result rows?",
            "baseline-inclusion rule",
            "same-scope metric summaries, prediction files, threshold records, and checksums",
            "Do not imply these omitted baselines were outperformed",
            ...
            "public release and matching commit",
            ...
            "identity, agenda, agenda-non-identity, and cannot-link feature groups",
            "derived risk score",
            "merge gate combines",
            ...
            "RoBERTa strong-baseline interpretation coverage",
            "main result section or cover letter could be read as dismissing the RoBERTa pair classifier",
            "claiming broad superiority from mixed row scopes",
            "RoBERTa pair classification is a strong baseline",
            "auditable relation-role design rather than a broad superiority claim",
            "pre-submission cover letter now keeps the same boundary",
            "RoBERTa pair classification as a strong supervised baseline",
            "strong-baseline interpretation, not new empirical evidence",
            ...
            "reviewer workload-transfer response coverage",
            "same-artifact workload evidence",
            "response discipline, not deployment evidence",
            "## Readiness Gate 142: Rule-Based Baseline Omission Response Gate",
            "rule-based and traditional-baseline omission response coverage",
            "artifact-readiness boundary",
            "comparison fairness, not stronger empirical evidence",
            "## Readiness Gate 143: Target-Route Reviewer Response Gate",
            "target-route reviewer response coverage",
            "DKE-style route is scope-fit preparation",
            "target-route discipline, not journal acceptance evidence",
            ...
            "public data-processing reproduction-boundary coverage",
            "Git-only review confirms L0 code check and L1 fixture rebuild",
            "cannot verify Open-v2/Open-v3 main-table numbers",
            "Full result audit requires L2 public-source rebuild or L3 result audit evidence",
            "`configs/source_input_manifest.json`, `logs/processing_run_log.jsonl`, `checksums.sha256`, and the external artifact release",
            "data-processing documentation discipline, not new numerical evidence",
            "does not provide raw third-party data, freeze public API responses, publish prediction files, or validate the missing L2/L3 artifacts",
            "## Readiness Gate 148: Candidate Official-Source Planning Gate",
            "candidate-route official-source planning coverage",
            "selected-target, ranking/category, template, and live-system claims",
            "target-journal shortlist now records DKE, Information Systems, and Scientometrics official-source checks on 2026-06-23",
            ...
            "Reviewer-response wording treats IAD-Risk as a conservative safety filter",
            ...
            "`docs/data-processing-pipeline.md` states that Git-only review confirms L0 code check and L1 fixture rebuild only",
            "python manuscript/scripts/validate_submission_package.py --final-upload --artifact-dir /path/to/release",
        ]
    )
...
def test_check_reviewer_readiness_audit_rejects_missing_main_result_strong_baseline_gate() -> None:
    """验证审稿准备度审计必须覆盖主结果强基线解释门禁。"""

    module = _load_validate_manuscript_module()
    audit_text = Path("manuscript/reviewer_readiness_audit.md").read_text(encoding="utf-8")
    for marker in [
        "Readiness Gate 138: Main-Result Strong-Baseline Interpretation Gate",
        "main-result strong-baseline interpretation drift",
        "RoBERTa strong-baseline interpretation coverage",
        "main result section or cover letter could be read as dismissing the RoBERTa pair classifier",
        "claiming broad superiority from mixed row scopes",
        "RoBERTa pair classification is a strong baseline",
    ...
    assert any("Main-Result Strong-Baseline Interpretation Gate" in error for error in errors)
    assert any("main-result strong-baseline interpretation drift" in error for error in errors)
    assert any("RoBERTa strong-baseline interpretation coverage" in error for error in errors)
...
def test_check_reviewer_readiness_audit_rejects_missing_workload_transfer_response_gate() -> None:
    """验证审稿准备度审计必须覆盖工作量转移回应门禁。"""

    module = _load_validate_manuscript_module()
    audit_text = Path("manuscript/reviewer_readiness_audit.md").read_text(encoding="utf-8")
    for marker in [
        "Readiness Gate 141: Reviewer Workload-Transfer Response Gate",
        "reviewer workload-transfer response drift",
        "reviewer workload-transfer response coverage",
        "Does IAD-Risk only move work from automatic merging to human review",
        "conservative safety filter",
        "throughput reduction",
    ...
    assert any("Reviewer Workload-Transfer Response Gate" in error for error in errors)
    assert any("reviewer workload-transfer response drift" in error for error in errors)
    assert any("conservative safety filter" in error for error in errors)


def test_check_reviewer_readiness_audit_rejects_missing_rule_based_baseline_omission_gate() -> None:
    """验证审稿准备度审计必须覆盖规则基线缺席回应门禁。"""

    module = _load_validate_manuscript_module()
    audit_text = Path("manuscript/reviewer_readiness_audit.md").read_text(encoding="utf-8")
    for marker in [
        "Readiness Gate 142: Rule-Based Baseline Omission Response Gate",
        "rule-based baseline omission response drift",
        "rule-based and traditional-baseline omission response coverage",
        "Why are exact identifier matching, title-normalization rules, or traditional entity-resolution systems not full main-result rows",
        "baseline-inclusion rule",
        "same-scope metric summaries, prediction files, threshold records, and checksums",
    ...
    assert any("Rule-Based Baseline Omission Response Gate" in error for error in errors)
    assert any("rule-based baseline omission response drift" in error for error in errors)
    assert any("baseline-inclusion rule" in error for error in errors)


def test_check_reviewer_readiness_audit_rejects_missing_target_route_response_gate() -> None:
    """验证审稿准备度审计必须覆盖目标路线回应门禁。"""

    module = _load_validate_manuscript_module()
    audit_text = Path("manuscript/reviewer_readiness_audit.md").read_text(encoding="utf-8")
    for marker in [
        "Readiness Gate 143: Target-Route Reviewer Response Gate",
        "target-route reviewer response drift",
        "target-route reviewer response coverage",
        "Why is the manuscript positioned toward Data & Knowledge Engineering rather than Information Systems or Scientometrics",
        "current scope-fit preparation, not final journal, ranking, or acceptance confirmation",
        "Do not claim selected-journal confirmation, Q2/B completion, or final-upload readiness",
    ...
    assert any("Target-Route Reviewer Response Gate" in error for error in errors)
    assert any("target-route reviewer response drift" in error for error in errors)
    assert any("DKE-style route is scope-fit preparation" in error for error in errors)


def test_check_reviewer_readiness_audit_rejects_missing_artifact_link_claim_upgrade_gate() -> None:
    """验证审稿准备度审计必须覆盖 artifact 链接主张升级边界。"""
...
def test_check_reviewer_readiness_audit_rejects_missing_final_cover_letter_sentence_map_gate() -> None:
    """验证审稿准备度审计必须覆盖最终投稿信句级映射边界。"""
    ...
    assert any("Final Cover-Letter Sentence Map Gate" in error for error in errors)
    assert any("final cover-letter sentence-map drift" in error for error in errors)
    assert any("concrete sentence map" in error for error in errors)


def test_check_reviewer_readiness_audit_rejects_missing_data_processing_git_only_boundary_gate() -> None:
    """验证审稿准备度审计必须覆盖数据处理 Git-only 复现边界。"""

    module = _load_validate_manuscript_module()
    audit_text = Path("manuscript/reviewer_readiness_audit.md").read_text(encoding="utf-8")
    for marker in [
        "Readiness Gate 147: Data-Processing Git-Only Reproduction Boundary Gate",
        "data-processing Git-only boundary drift",
        "public data-processing reproduction-boundary coverage",
        "Git-only review confirms L0 code check and L1 fixture rebuild",
        "cannot verify Open-v2/Open-v3 main-table numbers",
        "Full result audit requires L2 public-source rebuild or L3 result audit evidence",
    ...
    assert any("Data-Processing Git-Only Reproduction Boundary Gate" in error for error in errors)
    assert any("data-processing Git-only boundary drift" in error for error in errors)
    assert any("cannot verify Open-v2/Open-v3 main-table numbers" in error for error in errors)
...
def test_check_submission_material_quantitative_summary_rejects_unscoped_highlights() -> None:
    ...
    module = _load_validate_manuscript_module()
    highlights_text = "- IAD-Risk shows zero observed HNFMR."
    cover_letter_text = "\n".join(
        [
            "The manuscript reports an Open-v2 evidence snapshot.",
            "The result rows are scope-bounded mechanism evidence rather than a same-scope comparative ranking.",
    ...
    assert any("Open-v2 held-out hard-negative scope" in error for error in errors)
...
def test_check_final_upload_cover_letter_rejects_missing_declaration_sentence() -> None:
    """验证 final-upload 投稿信必须包含声明状态句。"""
    ...
    assert any("funding declaration boundary" in error for error in errors)
...
def test_check_final_upload_cover_letter_rejects_missing_evidence_boundary() -> None:
    """验证 final-upload 投稿信必须保留 Open-v2 证据边界。"""
    ...
    assert any("Open-v2 evidence boundary" in error for error in errors)
    assert any("scope-bounded mechanism wording" in error for error in errors)
    assert any("statistical-significance boundary" in error for error in errors)
